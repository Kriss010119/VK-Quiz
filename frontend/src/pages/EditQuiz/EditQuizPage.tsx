...
export const EditQuizPage = () => {
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      for (const question of questions) {
        const optionsForApi = question.options.map(opt => ({
          text: opt.text,
        ...
        if (question.isNew) {
          ...
          await api.patch(`/quizzes/${quizId}/questions/${question.id}`, {
            type: questionType,
            text: question.text,
            image_url: question.imageUrl || null,
            points: 100,
  ...
  if (isLoading) {
    return (
      <PageLayout sidebar={<Sidebar user={sidebarUser} />}>
        <Header />
        <div className={styles.loading}>Loading quiz data...</div>
        <Footer />
      </PageLayout>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className={styles.topMenu}>
        <BackLink onClick={() => navigate(`/quiz/${quizId}`)} />
        <h1 className={styles.pageTitle}>EDIT QUIZ</h1>
        <button
      ...
      <div className={styles.infoRow}>
        <div className={styles.infoBlock}>
          ...
          <div className={styles.formField}>
            ...
            <span className={styles.fieldValue}>{quiz?.room_code}</span>
          </div>
          ...
          <div className={styles.formField}>
            ...
            <span className={styles.fieldValue}>{quiz?.status?.toUpperCase()}</span>
          </div>
        </div>
